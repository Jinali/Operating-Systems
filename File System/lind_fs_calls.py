...
#Password Checking
  ...
  else raise SyscallError("Password entered incorrectly \n Cannot Proceed..")
...
#_________________________________________________________________________
def _blank_fs_init():
  pw = input("Enter a Password to Lock the filesystem.")
  encoded = pw.encode('tH1s1srEpYfsl0G9')
#_________________________________________________________________________
  ...
  filesystemmetadata['passwordprotection'] = {}
...
#________________________________________________________________________
  if filesystemmetadata['passwordprotection'] is not None:
    spwd = filesystemmetadata['passwordprotection']
    upw = input("This directory is password protected \n Please enter the password to access the Directory")
    if upw == spwd  
      # let's update and return success (0)
      fs_calls_context['currentworkingdirectory'] = truepath
    else 
      raise SyscallError("chdir_syscall","ENOENT","Access Denied")
#________________________________________________________________________
...
#_____________________________________________________________________
    ...
    #Some password Protected Folders
    choice = input("Do you want to password protect the Directory?\nY/ N")
    if (choice == 'Y'):
      pwd = input("Enter the Password to protect the Directory")
      #Just to make pwd unique for everyfolder, we would later compare dirname and password.
      encodeddir = pwd.encode(dirname)
      filesystemmetadata['passwordprotection'] = encodeddir
    else
      continue
#_____________________________________________________________________
...
def read_syscall(fd, count):
  ...
  try:
    ...
    data = fileobjecttable[inode].readat(1,position)
